function MGFRNN_main(dataset_name, data_index, ktimes)

%多分类GFRNN模型，对原有GFRNN在质量m与距离d的设置上有修改
...
dataname =  strcat('MGFRNN_',file_name);%转存的文件名
...
for i_cv = 1:ktimes    
   ...
   % 训练
   model = MGFRNN_train(train_all);
   % 测试
   [vec_res] = MGFRNN_test(test_all, train_all, model);
   %统计一轮的结果
