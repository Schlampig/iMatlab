function CRUSBoostRF_main(dataset_name, data_index, par)
...
for i_cv = 1:par.ktimes    
    ...
    % 训练&测试&记录
    [method, candi_vec, clf_name] = CRUSBoostRF_train(train_all, par);
    final_res(i_cv,:) = Ada_select_test(test_all, method, par, candi_vec);
    
end%for_i_cv

final_res(par.ktimes+1,:) = mean(final_res(1:par.ktimes,:));
final_res(par.ktimes+2,:) = std(final_res(1:par.ktimes,:));
final_name = strcat('CRUSBoostRF_',file_name,'_iter',num2str(par.T),'_k',num2str(par.k),'_ratio',num2str(par.ratio),clf_name);
