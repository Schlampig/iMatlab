function BEFLD_main(dataset_name, data_index,ktimes,dp_index,num_reg)

%标准BEFLD模型，包含正则化选择，及边界判断，将PILD基分类器替换为Fisher分类器
...
% 预处理
load(dataset_name);
dataname =  strcat('BEFLD_',Imbalanced_data{data_index,1},'_dp',num2str(dp_index),'_reg',num2str(num_reg));%转存的文件名
...
for i_cv = 1:ktimes
   ...
   % 训练
   [model,flag] = BEFLD_train(train_all,dp_index,num_reg);
   % 测试
