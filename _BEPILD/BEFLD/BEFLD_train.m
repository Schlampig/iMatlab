function [model,flag] = BEFLD_train(train_all, dp_index, num_reg)
...
X = [train_pos;train_neg];%POS类放在前面，NEG类放在后面，生成训练样本矩阵 NxD
...
% 使用Fisher（FLD）计算最初的超平面
w = inv(Sw + I_reg)*(m1 - m2)'; % Dx1
