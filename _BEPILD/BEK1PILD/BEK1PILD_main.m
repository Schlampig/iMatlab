function BEK1PILD_main(dataset_name, data_index, ktimes, par)

%核化BEPILD模型，包含正则化选择，及边界判断，但边界仍然是线性的
...
% 预处理
load(dataset_name);
dataname =  strcat('BEK1PILD_',Imbalanced_data{data_index,1},'_dp',num2str(par.dp),'_reg',num2str(par.reg),'_',par.ktype,'_',num2str(par.kpara));%转存的文件名
...
for i_cv = 1:ktimes
   ...
   % 训练
   [model,flag] = BEK1PILD_train(train_all,par);
   % 测试
