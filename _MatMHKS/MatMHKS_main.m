function MatMHKS_main(file_name,datanum,ktimes,ratio,C,u,b)
...
fprintf(file_id, 'Setting: MCCV_Time-%.1f ,Training_Ratio(Beta)-%.1f ,C-%.3f ,Dataset-%s \r\n', ktimes,ratio,C,dataname);
disp(['Setting: MCCV_Time-',num2str(ktimes),', Training_Ratio(Beta)-',num2str(ratio),' ,C-',num2str(C),' ,Dataset-',dataname]);%打印在屏幕上
fprintf(file_id,'--------------------------------------\r\n');%分隔符
disp(['--------------------------------------']);
    ...
    tic;%开始计算一次迭代的训练时间
        for i_classone = 1:(sum_class-1)
            for i_classtwo = (i_classone+1):sum_class
                ...
                MatStruct(i_classone,i_classtwo).candidate = MatMHKS_fun(train_binary_data,train_binary_label,InputPar,M_row,M_col);%代入训练，返回u和v
    ...
    time(ktimes) = toc;%第ktimes次MCCV训练用时
        ...
        for i_testone = 1:(sum_class-1)
            for i_testtwo = (i_testone+1):sum_class
                %依次代入两两类训练获得的数据进行测试          
                Group = MatMHKS_test(MatStruct(i_testone,i_testtwo).candidate,test_data_final,i_testone,i_testtwo,M_row,M_col);
        ...
        fprintf(file_id,'%f \r\n',accuracy(i_iter));
        disp(num2str(accuracy(i_iter)));%打印在屏幕上
    end%for_i_iter
