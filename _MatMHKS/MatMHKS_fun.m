function [MatStruct] = MatMHKS_fun(train_binary_data,train_binary_label,InputPar,M_row,M_col)

%计算MatMHKS的核心函数（注：在R2013a上运行，其他版本可能会有小错）
...
while (k_iter < total_iter)
    ...
    v(:,k_iter) = mat_get_v(train_binary_data,train_binary_label,InputPar,M_row,M_col,S2,I,u(:,k_iter),b(:,k_iter));
    e(:,k_iter) = mat_get_e(train_binary_data,train_binary_label,M_row,M_col,I,u(:,k_iter),v(:,k_iter),b(:,k_iter));
    ...
    if stop_tag < eta
        ...
        u(:,k_iter+1) = mat_get_u(train_binary_data,train_binary_label,InputPar,M_row,M_col,S1,I,v(:,k_iter),b(:,k_iter));
    end%end_if    
    k_iter = k_iter + 1;
