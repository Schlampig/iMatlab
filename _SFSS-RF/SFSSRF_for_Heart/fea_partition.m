...
data = data_now(:,1:end-1); %提取出数据部分
label = data_now(:,end); %提取出类别标记部分
f_cell{1,1} = [data(:,1:end-1),label]; % preserve all
f_cell{1,2} = [data(:,2:end-1),label]; % no age
f_cell{1,3} = [data(:,[1,3:end-1]),label]; % no gender
f_cell{1,4} = [data(:,[1:2,4:end-1]),label]; % no heart ratio
f_cell{1,5} = [data(:,[1:3,148:end-1]),label]; % no diagnose
f_cell{1,6} = [data(:,[1:147,159:end-1]),label]; % no medications
f_cell{1,7} = [data(:,1:158),label]; % no laboratory tests
