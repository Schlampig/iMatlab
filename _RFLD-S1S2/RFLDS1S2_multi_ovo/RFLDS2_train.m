function [clf] = RFLDS2_train(train_all, par)
...
X = train_all(:,1:end-1);%POS类放在前面，NEG类放在后面，生成训练样本矩阵 NxD，无标号
...
elseif num_min_pos == num_max_neg % 正负类相交的地方正好是边界，以边界为中心作新直线
    ...
    clf.bpos = bpos;
    clf.bneg = bneg;
    clf.candi_dis_pos = candi_dis_pos;
    clf.candi_dis_neg = candi_dis_neg;
end%if
