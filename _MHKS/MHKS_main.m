function MHKS_main(file_name,datanum,ktimes,ratio,C,b)
    ...
    tic;%开始计算一次迭代的训练时间
        for i_classone = 1:(sum_class-1)
            for i_classtwo = (i_classone+1):sum_class
                ...
                MHKSStruct(i_classone,i_classtwo).candidate = MHKS_fun(train_binary_data,train_binary_label,InputPar);%代入训练，返回w和b
    ...
    time(ktimes) = toc;%第ktimes次MCCV训练用时
        ...
        for i_testone = 1:(sum_class-1)
            for i_testtwo = (i_testone+1):sum_class
                %依次代入两两类训练获得的数据进行测试          
                Group = MHKS_test(MHKSStruct(i_testone,i_testtwo).candidate,test_data_final,i_testone,i_testtwo);
