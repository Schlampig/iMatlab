function CIMatMHKS_main(file_name,datanum,ktimes,ratio,C,dc,attr_num,u,b)
...
fprintf(file_id, 'Setting: MCCV_Time-%.1f ,Training_Ratio(Beta)-%.1f ,C-%.3f ,RadiusRatio-%.2f ,SelectedAttr-%.2f ,Dataset-%s \r\n', ktimes,ratio,C,dc,attr_num,dataname);
disp(['Setting: MCCV_Time-',num2str(ktimes),', Training_Ratio(Beta)-',num2str(ratio),' ,C-',num2str(C),' RadiusRatio-',num2str(dc),' SelectedAttr-',num2str(attr_num),' ,Dataset-',dataname]);%打印在屏幕上
fprintf(file_id,'--------------------------------------\r\n');%分隔符
disp(['--------------------------------------']);
    ...
    tic;%开始计算一次迭代的训练时间
        for i_classone = 1:(sum_class-1)
            for i_classtwo = (i_classone+1):sum_class
                ...
                tempt_candiattr_one = FNN_fun(train_classone_data,dc,attr_num);%使用FC对属性聚簇，返回每一类的代表属性
                tempt_candiattr_two = FNN_fun(train_classtwo_data,dc,attr_num);
