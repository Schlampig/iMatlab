function [PMatStruct] = WPMatMHKS_fun(train_binary_data,train_binary_label,InputPar,M_row,M_col,wpc)

%计算WPMatMHKS的核心函数（注：在R2013a上运行，其他版本可能会有小错）
...
%生成lij关系矩阵M
M = train_binary_label*train_binary_label';
D = WPC_generate(train_binary_data,n1,n2,wpc);

%开始迭代训练
while (k_iter < total_iter)
          
    v(:,k_iter) = wpmat_get_v(train_binary_data,train_binary_label,InputPar,M_row,M_col,S2,I,u(:,k_iter),b(:,k_iter),M,D);
    e(:,k_iter) = wpmat_get_e(train_binary_data,train_binary_label,M_row,M_col,I,u(:,k_iter),v(:,k_iter),b(:,k_iter));
    ...
    if stop_tag < eta
        ...
        u(:,k_iter+1) = wpmat_get_u(train_binary_data,train_binary_label,InputPar,M_row,M_col,S1,I,v(:,k_iter),b(:,k_iter),M,D);
    end%end_if    
    k_iter = k_iter + 1;
...
if k_iter == total_iter
    PMatStruct.u = u(:,k_iter);
    PMatStruct.v = v(:,k_iter-1);
else
    PMatStruct.u = u(:,k_iter);
    PMatStruct.v = v(:,k_iter);    
end%end_if
